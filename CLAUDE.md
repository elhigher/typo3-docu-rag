...
1. **`fetch-docs.ts`** — clones 4 TYPO3 doc repos (branch `13.4`) into `data/raw/` using `simple-git`. Pulls on re-run.
...
- **`search_docs(query, limit?, repo?)`** — Small-to-Big retrieval: vector-searches the `chunks` table, then resolves matched chunks back to their `parents` for full-context responses. `repo` filters to `CoreApi`, `Typoscript`, `TCA`, or `Fluid`.
