...
function parseHtmlToSections(htmlContent, filePath, repoName) {
...
function createParentDoc(title, content, source, url, repo, index) {
    const fullContent = `${title}\n${content}`;
    const parentId = generateId(`${repo}-${source}-${title}-${index}`);
    const paragraphs = content.split(/\n\n+/).filter(p => p.trim().length > 50);
    const chunks = paragraphs.map((p, chunkIndex) => ({
