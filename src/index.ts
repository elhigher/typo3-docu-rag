...
const BEST_PRACTICES_VERSIONS: Record<string, string> = {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    ...
    if (name === "get_best_practices") {
      ...
      const versionsToInclude = BEST_PRACTICES_VERSION_ORDER.slice(
        0,
        BEST_PRACTICES_VERSION_ORDER.indexOf(version) + 1
      );
