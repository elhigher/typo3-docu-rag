...
export async function buildIndex(docsPath: string, dbDir: string, embedder: any): Promise<void> {
  const allDocs: ParentDoc[] = JSON.parse(fs.readFileSync(docsPath, 'utf-8'));
  ...
  const chunkRecords = allDocs.flatMap(doc =>
